...
class SRHF():
    ...
    def run(self):
        ...
        H = T + V
        #if not self.options.intsdpd:
        #    repacked_bigERI = symmetry_adapt.rank4_transform(bigERI)
        #    repacked_bigERI_swapped = symmetry_adapt.rank4_transform(np.swapaxes(bigERI, 1, 2))
        #    #repacked_bigERI = symmetry_adapt.rank4_transf_alt(bigERI)
        #    #repacked_bigERI_swapped = symmetry_adapt.rank4_transf_alt(np.swapaxes(bigERI, 1, 2))
        #    #print("repacked_bigERI")
        #    #for eri in repacked_bigERI:
        #    #    print(eri.shape)
        #    #    print(eri)
        #    #print("repacked_bigERI_swapped")
        #    #for eri in repacked_bigERI_swapped:
        #    #    print(eri.shape)
        #    #    print(eri)
        #else:
        #    bigERI = self.aotoso_2(bigERI)
      
        if not self.options.benchmark:
            print("Repacking and symmetry blocking ERI")
            before = time.time()
            #ERI_swapped = np.swapaxes(bigERI, 1, 2)
            #self.dpd = DPD(self.salcs.salcs_by_irrep, self.symtext, self.salcs, so_orbitals.fxn_list, D_i, self.options)
            self.dpd = DPD(self.salcs.salcs_by_irrep, self.symtext, self.salcs, so_orbitals, D_i, self.options)
            #self.dpd.blocks()
            #self.dpd.alternate(ints.ao_eri().np)
            #repacked_bigERI = self.dpd.trial
            #
            #self.dpd.alternate_swap(ints.ao_eri())
            ...
            for i in range(1, self.options.scf_max_iter + 1):
                ...
                if i > 3:
                    self.fock_timings.append(ftime)
                print("The fock and coefficients to feed into Second Order Step")
                print("Fock")
                print(F)
                print("Coeffs")
                print(C)
                ...
                #everything below this line should be thrown into a function
                #diag step, diis step, and newton step for each iteration
                #Psi4numpy only takes one diag step, then performs diis until error is below minimum, newton steps to finish

                #dRMS is the density error for convergence criteria. Density matrix is only explicitly built in fock step.
                #orbitals are updated in SO-step, hence no diagonalization. Those updated orbitals build the new density for the Fock construction
                ...
                #D_new, docc_vector = degen_build_D(C, eps, so_orbitals)
                #D = build_D(C, eps, ndocc)
                #compute delta e, update E_i
                ...
                #delta_d = D_now - D_i
                now = time.time()
                print(f"Iter {i:>3} SCF energy {E_new:>.10f} Delta(E) {delta_e} RMS(D) {d_rms} DIIS {diis_m.diis.dRMS} {docc_vector} took {now - before:.7f} seconds")
                ...
                if i == self.options.scf_max_iter:
                    print(f"SCF Cycles Did Not Converge In {i} Iterations, Harass the Developer(s) about your issue.")
                    print(self.fock_timings)
                    print(len(self.fock_timings))
                    self.avg_ftime = np.average(self.fock_timings)
                    print(self.symtext.chartable.irreps)
                    self.compute_psi(E_new)

    def second_order(self, F, Cs, docc_vector):
    ...
    def build_fock_blocky_sym(self, H, Dp, repacked_bigERI, repacked_bigERI_swapped):
        ...
        now = time.time() 
        #print(f"One electron operator broadcasting took {now - before:6.3f} seconds")
        fstart = time.time()
        jktime_total = 0
        #print(self.dpd)
        for b, block in enumerate(self.dpd.nonzero_blocks):
            ...
            #form j and k 
            jkstart = time.time()
            #print(repacked_bigERI[b].shape)
            #print(repacked_bigERI_swapped[b].shape)
    ...
    def jk(self, ERI,ERI_swap, d, braket, block):
        #print(f"The braket condition {braket}")
        #print(f"Block {block}")
        #print("Inside JK")
        #print(f"ERI")
        #print(ERI) 
